...
FlipTracker continues through individual batch failures and leaves failed rows available for correction and retry. Actual used-item photos are uploaded to eBay Picture Services when the draft is sent. Books can request an eBay catalog match by ISBN; other used media requires an actual photo. FlipTracker does not call eBay's publish endpoint, so review every offer before publishing in Seller Hub.
