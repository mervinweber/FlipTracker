...
Book drafts use ISBN-based eBay catalog matching instead of forwarding third-party metadata image URLs. Media drafts default Language to English but expose a per-listing selector. Older free-form Language specifics populate the selector; after the listing is saved, the structured selection is authoritative. Inventory item creation removes duplicate UPC/EAN/ISBN aspects and retries eBay error `25001` once with core product fields plus Language when present, preserving the original detailed error if the reduced request also fails.
