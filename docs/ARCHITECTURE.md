...
Book drafts use ISBN-based eBay catalog matching instead of forwarding third-party metadata image URLs. Media drafts default Language to English but expose a per-listing selector. Book Title is a separate required eBay aspect that defaults to the inventory title. Older free-form values populate the structured fields; after the listing is saved, the structured selections are authoritative. Inventory item creation removes duplicate UPC/EAN/ISBN aspects and retries eBay error `25001` once with core product fields plus required Language and Book Title aspects when present, preserving the original detailed error if the reduced request also fails.
