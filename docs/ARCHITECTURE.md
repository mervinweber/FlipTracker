...
`sourcingAnalyses` stores the saved inputs and calculated decision snapshot. `sourcingComps` stores each observed sold price separately, including item price, shipping, and delivered total. The pure calculation layer in `convex/lib/sourcingRules.ts` calculates median and average sold price, sell-through proxy, estimated days to sell, rarity, liquidity, expected fees, profit, ROI, confidence, Buy / Maybe / Pass, and the maximum purchase cost that satisfies the seller's minimum-profit and ROI rules.
...
## Deferred Vinted Wardrobe Prototype

```text
Existing Inventory Asset -> crossListings(platform=Vinted) -> filtered wardrobe workspace -> direct Vinted item URL
```

The dormant Vinted workspace is a focused index over the shared inventory model. `linkedAccounts` can store an optional account label, username, browser login URL, and wardrobe/profile URL. It never stores a Vinted password. `crossListings` can associate one physical `assets` record with its Vinted title, category, status, price, condition, notes, external item ID, and normalized direct listing URL.

The prototype derived category counts and filtered views from seller-maintained Vinted links. It was removed from application routing and navigation because browser login does not provide inventory access, making the workflow too manual for its primary purpose.

Standard Vinted accounts are not scraped, crawled, or controlled by FlipTracker. Vinted's official Pro Integrations API uses allowlisted business access and server-side signed requests rather than consumer OAuth, and it is not available for this US workflow. If approved US access becomes available later, a Convex integration may import catalog items and reconcile statuses into the existing `crossListings` design before the tab is restored.
...
The Listings view can also import the authenticated seller's active eBay listings through Trading API `GetMyeBaySelling`. Records are deduplicated by eBay item ID and labeled by creation model: `FlipTracker API`, `eBay app / Seller Hub`, or `FlipTracker draft`. Native eBay fixed-price listings use Trading API `ReviseInventoryStatus` for price changes and `EndFixedPriceItem` for endings; Inventory API listings keep using their Offer ID. Seller Hub/app drafts are not returned by `GetMyeBaySelling`, so they cannot be imported until they are active or scheduled. Broader title, description, category, condition, shipping, and photo revisions for native listings remain intentionally local-only until a GetItem/ReviseFixedPriceItem merge can preserve every field safely.
...
The same marketplace listing continues through a separate fulfillment state: Awaiting Shipment, Packed, Shipped, or Completed. eBay synchronization advances paid orders to Awaiting Shipment or Shipped from the order response; seller actions retain package measurements, service, carrier, tracking, insurance, label cost, notes, and packed/shipped timestamps. The dedicated Shipping view is a projection over open fulfillment states and can print a bin-sorted pick list without creating duplicate task records. Outbound label purchase remains a Seller Hub Labels handoff because eBay does not expose a general public seller-label purchase endpoint. After purchase, `submitShippingFulfillment` preflights the order and existing fulfillments, then calls `createShippingFulfillment` only when the line item has not already been submitted. The resulting tracking, fulfillment ID, shipment time, label cost, sale accounting, and lifecycle event remain attached to the original record. This write path requires `sell.fulfillment`, so existing sellers must reconnect once after deployment.
