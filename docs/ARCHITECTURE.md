...
The same marketplace listing continues through a separate fulfillment state: Awaiting Shipment, Packed, Shipped, or Completed. eBay synchronization advances paid orders to Awaiting Shipment or Shipped from the order response; seller actions retain carrier, tracking, insurance, notes, and packed/shipped timestamps. The Today queue is a pure deterministic projection over listing readiness, eBay reconciliation, active age, and fulfillment state, so it creates no duplicate task records and always opens the original listing workflow.
