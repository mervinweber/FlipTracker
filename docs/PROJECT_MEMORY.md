...
- The main Listings workspace is organized by item lifecycle: Queue, Active, Sold, and Needs Attention. Each view exposes only its relevant metrics, filters, synchronization commands, and primary row action; secondary commands live in a More menu. Seller Connection is collapsed after setup so operational records remain the visual focus.
- Long eBay API errors never render inline in Listings rows. Rows show a compact `eBay issue` indicator that opens the complete message and a direct Review Listing action in a mobile-safe dialog.
...
- Bulk Intake also supports reviewed Photo Lots for up to 12 video games. One group image is sent to Gemini for title/platform identification, conservative working price ranges, and editable eBay copy; the seller reviews every row, total purchase cost is allocated exactly in cents, and one Convex mutation creates the linked inventory records and internal drafts. The source image is identification-only, so used games still enter the cross-device Photos queue for actual listing photos.
...
- Sold records bypass eBay category, photo, shipping, and item-specific readiness checks. Imported eBay sales remain in Operations only until acquisition cost is entered; Operations and Edit open Record Sale directly. Completed fulfillment is displayed as Archived while the record remains Sold for revenue and profit reporting.
...
- Stale Listing Manager applies .99 charm pricing only when the rounded result is below the current price. Listings already ending in .99 retain the exact requested percentage reduction instead of being incorrectly excluded as unchanged, and every exclusion reason is included in the summary count.
...
- The Cards workspace now runs a rapid Pokemon/Yu-Gi-Oh! session: front/back photos are saved with each asset, exact duplicates are grouped in-session, and deterministic Individual/Playset/Bundle/Hold guidance uses the seller's minimum single-card value. Vinted-ready mode creates inventory and copy for manual posting only; it never logs into or automates Vinted.
- Yu-Gi-Oh! set codes are not sufficient exact-print identifiers because some products issue the same code in several rarities. Detailed YGOPRODeck/TCGplayer-backed set variants are separate candidates and rarity is included in duplicate grouping. Gemini rarity detection remains a non-authoritative clue that always requires seller confirmation.
- Pokemon collector numbers are also not sufficient exact-print identifiers. Pokemon TCG API price finishes plus free TCGCSV pattern products separate normal, holofoil, reverse holofoil, Poke Ball reverse holo, and Master Ball reverse holo candidates. Finish remains part of the duplicate key and image-based finish suggestions require seller confirmation.
- Card-front capture automatically runs catalog lookup from Gemini-extracted identifiers. One unambiguous match is preselected; multiple rarity/finish/edition candidates require seller choice. Provider price zeroes mean no usable quote and must display as missing, not `$0.00`.
- Yu-Gi-Oh! photo extraction also attempts to read explicit `1st Edition` text and the bottom `1996`/`2020` copyright year. Candidate ranking uses those marks to distinguish original Unlimited copies from `2020 Date Reprint` entries, but the seller must confirm the tiny print.
