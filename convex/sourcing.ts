...
import { calculateAnalysis, type SourcingCompInput } from "./lib/sourcingRules";
...
async function insertAnalysis(
  ...
  if ([input.purchaseCost, input.shippingCost, input.packagingCost, input.feePercent, input.targetProfit ?? 0, input.targetRoiPercent ?? 0, input.minimumLiquidity ?? 0, input.activeCount, input.soldCount90].some((value) => value < 0)) {
  ...
  const analysisId = await ctx.db.insert("sourcingAnalyses", {
    ...
    targetProfit: round(input.targetProfit ?? 10),
    targetRoiPercent: round(input.targetRoiPercent ?? 50),
    minimumLiquidity: round(Math.min(100, input.minimumLiquidity ?? 40)),
